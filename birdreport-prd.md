...
**变更记录**
- **v2.3（本次）：** 按「用户视角 / 功能转化视角 / 工程视角」三个维度重组全文结构；将用户画像从功能描述中剥离单列；数据层拆分为「数据定义」与「数据应用」两节；其余数据内容与 v2.2 一致。
...
观鸟者出行前的心智状态，本质上只有三种——手里攥着两个已知变量，想求第三个：

| 心态 | 已知 | 想求 | 典型说法 |
|---|---|---|---|
| **休闲野观（逛）** | 地点 ＋ 时间 | 能看到什么鸟 | "我下周去香格里拉，有什么可以看？" |
| **狙击目标鸟** | 地点 ＋ 目标鸟种 | 几月去最合适 | "我要拍黑颈鹤，纳帕海几月去概率最高？" |
| **按鸟选目的地** | 出行时段 ＋ 目标鸟种 | 去哪个点最稳 | "12 月想拍黑颈鹤，云南哪个点最值得去？" |

这三种心态**不是三类人群**，同一个鸟友在不同行程里会切换角色；页面命名也沿用这个语气——**看什么 / 何时去 / 去哪看**，而不是「Bar Chart / 组合 N」之类的内部术语。
...
- 时间表达**模糊**："下个月""11 月底""冬天"——不会精确到某一天。
- 隐含的问题往往是**"值不值得去"**，而不仅是"有没有"。

**设计含义：** 地点搜索需支持点位俗名模糊匹配；时间槽需以月 / 旬为粒度而非日历选择器；结果需要给出"值得专程" / "顺路碰运气"的倾向性判断。
...
**三路径共同终点：点位详情页**
历史记录列表、报告人、出片数、最佳出行日期——三扇门进同一个房间。
